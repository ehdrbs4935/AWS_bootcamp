...
---
### 6. IAM: 권한 부여 
> * 사용자, 그룹 또는 역할에 권한을 할당하려면 IAM 정책을 생성하거나 계정의 기존 정책을 찾아야 함. (기본 권한은 없음.)  
> * 계정에서 사용자에게 명시적으로 허용되지 않은 모든 작업은 기본적으로 거부됨.(명시적 거부)  
> * 명시적으로 허용하지 않은 작업 또한 모두 거부되고, 명시적으로 거부한 작업은 항상 거부됨.  

> * **최소 권한의 원칙**은 컴퓨터 보안에서 중요한 개념임.  
> * 이 원칙은 사용자의 요구 사항에 따라 사용자에게 필요한 최소한의 사용자 권한만 부여하도록 함. IAM 정책을 생성할 때는 최소 권한을 부여하는 이 보안 조언을 따르는 것이 모범 사례임.  
> * 사용자가 할 수 있어야 하는 작업을 결정한 다음 이러한 작업만 수행할 수 있게 하는 정책을 작성함.  
> * 최소한의 권한 세트로 시작하여 필요에 따라 추가 권한을 부여함.  
> * 이 방법은 너무 넓은 범위의 권한에서 시작한 후 나중에 부여된 권한을 잠그는 방법보다 안전함.  

> * IAM 서비스 구성의 범위는 **글로벌**임.  
> * 설정은 AWS 리전 수준에서 정의되지 않으며, 모든 AWS 리전에 적용됨.  
---
### 6. IAM 정책
> * IAM 정책은 **엔터티에 부여될 권한을 공식적으로 설명하는 것**임. 정책은 모든 IAM 엔터티에 연결될 수 있음.  
> > 엔터티에는 **사용자, 그룹, 역할 또는 리소스**가 포함됨.  
> * 예를 들어, 승인된 IP 주소 범위에서 수신된 요청이 아닌 모든 요청을 차단하는 정책을 AWS 리소스에 연결할 수 있음.  
> * 정책은 허용되는 작업, 작업을 수행할 수 있는 리소스 및 사용자가 리소스에 대한 액세스를 요청할 때의 결과를 지정함.  

> * 정책이 평가되는 순서는 평가 결과에 영향을 주지 않음. 모든 정책이 평가되며, 결과는 항상 요청을 허용하거나 거부하는 것임.  
> * **충돌이 있는 경우 가장 제한적인 정책이 적용됨.**  

> * IAM 정책에는 2가지 유형이 있음.  
> **1) 자격 증명 기반 정책**  
> > * 자격 증명 기반 정책은 IAM 사용자, 역할 또는 그룹과 같은 보안 주체(또는 자격 증명)에 연결할 수 있는 권한 정책임.  
> > * 이러한 정책은 자격 증명으로 수행할 수 있는 작업, 대상 리소스 및 관련 조건을 제어함.  
> > * 자격 증명 기반 정책은 추가로 다음과 같이 분류될 수 있음.  
> > > * **관리형 정책**: AWS 계정의 여러 사용자, 그룹 및 역할에 연결할 수 있는 독립 실행형 자격 증명 기반 정책  
> > > * **인라인 정책**: 사용자가 생성하고 관리하며, 단일 사용자 그룹 또는 역할에 직접 포함되는 정책  

> **2) 리소스 기반 정책**
> > * 리소스 기반 정책은 S3 버킷과 같은 리소스에 연결하는 JSON 정책 문서임.  
> > * 이러한 정책은 지정된 보안 주체가 
